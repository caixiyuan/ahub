...
describe Ahub::Followable do
  ...
  describe '#followers' do
    it 'returns exmpty array when node_id is not specified' do
      tester = Ahub::FollowableTester.new({id: nil})
      expect(tester.followers).to eq([])
    end

    it 'returns correct followers' do
      tester = Ahub::FollowableTester.new({id: 1000})
      allow(Ahub::FollowableTester).to receive(:get_resource).with(
        url: "#{Ahub::FollowableTester.base_url}/#{tester.id}/followers.json?page=0&pageSize=50",
        headers: Ahub::FollowableTester.admin_headers,
        klass: Ahub::User
